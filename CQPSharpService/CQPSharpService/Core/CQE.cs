...
namespace CQPSharpService.Core {
    ...
    public static class CQE {
        ...
        public static string GetQQName(long qqNumber) {
            nLogger.Info(string.Format("[↓][昵称] QQ：{0}", qqNumber));
        ...
        public static List<CQGroupInfo> GetGroupList() {
            ...
            try {
                ...
                foreach (Match match in matches) {
                    ...
                    lst.Add(gInfo);
                    nLogger.Info(string.Format("Name: {0}, Number: {1}, Owner: {2}", gInfo.Name, gInfo.Number, gInfo.Owner));
                }

            } catch (Exception e) {
                nLogger.Error(e, "GetGroupList " + e.Message);
                lst = null;
            }
