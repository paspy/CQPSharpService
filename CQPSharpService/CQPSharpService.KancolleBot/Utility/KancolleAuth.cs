    ...
    public sealed class KancolleAuth {
        ...
        private Tuple<string, string> GetDMMTokensForAjaxAsync() {
            try {
                ...
                var htmlResult = HttpHelper.Get(AuthURLs["login"]);
        ...
        private Tuple<string, string, string> GetAjaxTokensAsync(string dmm_token, string token) {
            try {
                WebHeaderCollection headers = new WebHeaderCollection();
                headers.Add("Origin", "https://www.dmm.com");
                headers.Add("Dmm_Token", dmm_token);
                headers.Add("X-Requested-With", "XMLHttpRequest");
                CookieCollection cookies = null;
                var jsonResult =
                    HttpHelper.Post(
                        AuthURLs["ajax"], new Dictionary<string, string>() { { "token", token } },
                        AuthURLs["login"], headers, ref cookies, Encoding.UTF8);
        ...
        private string GetOSAPIUrlAsync(string token, string idKey, string pwdKey) {
            try {
                using (var httpReqMsg = new HttpRequestMessage()) {
                    ...
                    var r = (m_client.SendAsync(httpReqMsg).Result).Content.ReadAsStringAsync().Result;
                    var pwdResetTest = AuthPattens["reset"].Match(r);
                    //if need pwd reset
                    if (pwdResetTest.Success) {
                        throw new Exception("DMM needs you to reset password!");
                    }
                }
                var response = m_client.GetAsync(AuthURLs["game"]).Result;
                var htmlResult = response.Content.ReadAsStringAsync().Result;
                var osapiReslt = AuthPattens["osapi"].Match(htmlResult);
        ...
        private Tuple<int, string, string> GetKancolleServerTokenAsync(string osapiUrl) {
            try {
                ...
                using (var httpReqMsg = new HttpRequestMessage()) {
                    ...
                    var result = (m_client.SendAsync(httpReqMsg).Result).Content.ReadAsStringAsync().Result;
                    result = result.Substring(7);
                    dynamic svdata = JToken.Parse(result);
                    if (svdata.api_result == 1) {
                        worldId = svdata.api_data.api_world_id;
                        worldIP = KCBManager.SERVER_ADDRESS[worldId - 1];
                    } else
